...
function setAuth(auth) { sessionStorage.setItem(AUTH_KEY, auth ? '1' : '0'); }
...
async function deriveHash(value) {
  ...
  const bits = await crypto.subtle.deriveBits({ name: 'PBKDF2', salt, iterations: AUTH.iterations, hash: 'SHA-256' }, keyMaterial, 256);
...
async function loginHandler(e) {
  ...
  if (constantTimeEqualBase64(uHash, AUTH.usernameHash) && constantTimeEqualBase64(pHash, AUTH.passwordHash)) {
    clearLockout();
    setAuth(true);
    routeView();
...
async function addContentHandler(e) {
  ...
  try {
    [imageMeta, materialMeta] = await Promise.all([
      uploadFile(imageFile && imageFile.size ? imageFile : null),
      uploadFile(materialFile && materialFile.size ? materialFile : null)
    ]);
  } catch (err) {
    notify('adminNotice', `Ошибка загрузки файла: ${err.message}`, false);
    return;
  }
  ...
  const item = {
    ...
    image: imageMeta?.fileUrl || '',
    mapPath: String(formData.get('mapPath') || '').trim(),
    fileName: materialMeta?.originalName,
    fileType: materialMeta?.fileType,
    fileUrl: materialMeta?.fileUrl
  };
...
function renderDeleteList() {
  ...
  byId('deleteList').innerHTML = list.length
    ? list.map(item => `<article class="card"><h3>${item.title}</h3><p class="meta">${item.category || 'Общее'}</p><button class="btn" data-delete-id="${item.id}" data-delete-type="${type}">Удалить</button></article>`).join('')
...
function initAdmin() {
  ...
  byId('modeDelete').addEventListener('click', () => { setMode('delete'); renderDeleteList(); });
