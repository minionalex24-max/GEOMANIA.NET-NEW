...
const seedData = {
  ...
  presentations: [{ id: crypto.randomUUID(), title: 'Реки Азии', category: 'Гидрология', classLevel: '8 класс', description: 'Крупнейшие речные системы.' }],
  ...
  films: [{ id: crypto.randomUUID(), title: 'Планета океанов', category: 'Природа', description: 'Документальный фильм об экосистемах.' }],
...
const sourceLabel = { maps: 'Карты', articles: 'Статьи', news: 'Новости', presentations: 'Презентации', films: 'Фильмы', exams: 'Экзамены' };
...
function downloadMarkup(item) { return item.fileUrl ? `<button class="btn" data-download="${item.id}">Скачать</button>` : ''; }
function mapOpenMarkup(item) { return item.mapPath || item.image ? `<button class="btn map-open-btn" data-open-map="${item.id}">Открыть карту</button>` : ''; }

function cardTemplate(item, type) {
  const mapBtn = type === 'Map' ? mapOpenMarkup(item) : '';
  const readBtn = type === 'Article' ? '<button class="btn" data-read-article="1">Открыть статью</button>' : '';
  const downloadBtn = type !== 'Map' ? downloadMarkup(item) : '';
  const tags = (item.tags || []).map(t => `<span class="tag">${t}</span>`).join('');
  return `<article class="card" id="item-${item.id}" data-type="${type}" data-id="${item.id}">${imageMarkup(item)}<h3>${item.title}</h3><p class="meta">${typeLabel[type]} · ${item.category || 'Общее'} ${item.difficulty ? `· ${item.difficulty}` : ''} ${item.classLevel ? `· ${item.classLevel}` : ''}</p><p>${item.description}</p>${tags}<div class="toolbar">${mapBtn}${readBtn}${downloadBtn}</div></article>`;
}
...
function setupGlobalSearch() {
  ...
  input.addEventListener('input', () => {
    ...
    const results = sources.flatMap(source => state.data[source].map(item => ({ ...item, source })).filter(i => `${i.title} ${i.description} ${(i.tags || []).join(' ')} ${i.category || ''}`.toLowerCase().includes(q)));
    box.innerHTML = results.slice(0, 12).map(r => `<a class="search-link" href="${(pageMap[r.source] || 'index.html')}#item-${r.id}"><div class="card">${imageMarkup(r)}<p class="meta">${sourceLabel[r.source] || r.source}</p><h3>${r.title}</h3><p>${r.description}</p></div></a>`).join('') || '<p class="muted">Совпадений нет.</p>';
    box.classList.remove('hidden');
  });
}

function ensureTextModal() {
  if (document.getElementById('textModal')) return;
  const wrapper = document.createElement('div');
  wrapper.id = 'textModal';
  wrapper.className = 'modal hidden';
  wrapper.innerHTML = '<div class="modal-box text-modal-box"><button id="closeTextModal" class="btn modal-close">Закрыть</button><h3 id="textModalTitle"></h3><pre id="textModalContent" class="text-modal-content"></pre></div>';
  document.body.appendChild(wrapper);
  document.getElementById('closeTextModal').addEventListener('click', () => wrapper.classList.add('hidden'));
}

async function decodeTextContent(item) {
  const res = await fetch(item.fileUrl);
  return await res.text();
}

function isTextLikeFile(item) {
  const t = (item.fileType || '').toLowerCase();
  const n = (item.fileName || '').toLowerCase();
  return t.startsWith('text/') || ['.txt', '.md', '.csv', '.json', '.xml', '.log'].some(ext => n.endsWith(ext));
}
...
function setupActions() {
  document.addEventListener('click', async (e) => {
    ...
    if (type === 'Map') {
      ...
      const modalImg = document.getElementById('mapModalImage');
      if (!modal || !modalImg) { window.location.href = `maps.html#item-${id}`; return; }
      modalImg.src = src;
    ...
    if (['Presentation', 'Film', 'Article', 'News', 'Exam'].includes(type)) {
      ...
      if (type === 'Article' && !item.fileUrl) {
        ensureTextModal();
        document.getElementById('textModalTitle').textContent = item.title;
        document.getElementById('textModalContent').textContent = item.description;
      ...
      if (isTextLikeFile(item)) {
        ensureTextModal();
        document.getElementById('textModalTitle').textContent = item.title;
        document.getElementById('textModalContent').textContent = await decodeTextContent(item);
        document.getElementById('textModal').classList.remove('hidden');
        return;
      }
